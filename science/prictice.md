...
- **数据的物理结构**是指数据在计算机内存储的实际形式。
- 数据的物理结构包括**数据元素**的表示和**数据元素间关系**的表示。
- 数据结构是研讨数据的**逻辑结构和物理结构**，以及它们之间的相互关系，并对这种结构相应的**运算**，设计出相应的**算法**。
...
- 两个栈**共享一个内存空间**时，把它们栈底分别设在空间地址两端。
...
>（1）解析：$1TB=2^{40}B,1TB/1KB=2^{32}$个盘块，所以盘块号要用32位，即4个字节长表示，512B/4B=128个索引项，因此能表示的最大文件=128 x1KB=128KB。
...
|  网络3   |  1   |  F   |
|  网络4   |  5   |  G   |
A到网1原要四跳，但C到网1只要2跳，A到C1跳，所以更新其为2+1=3跳，下一跳为C。
...
- （1）写出f(x)的**带拉个郎日余项**的一阶麦克劳林公式：`f(x)=f(0)+f'(0)x+f''(c)x^2/2!`，c在(0,x)之间。
...
题1.4：下面左侧矩阵A，E为3阶单位矩阵求，AB=E中的矩阵B。<i tag=l1>矩阵的一个求法</i>
