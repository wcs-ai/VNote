...
### 3、完形填空：
**trick**：<b c=r>建议先读完一段或两段了解大意，做一些简单的猜想，推断后再看具体每一空，根据上下文语义，是否呼应，固定搭配等来选词。</b>
...
**试题资源**：[资源1](http://www.offcn.com/kaoyan/shiti/2572/)、[当前试题](http://www.offcn.com/kaoyan/2020/1229/194900.html)、
...
题 5：若$lim_{x\rightarrow 0}\frac{sin6x+xf(x)}{x^3}=0,求lim_{x\rightarrow 0}\frac{6+xf(x)}{x^2}$
...
**题8**：函数f(x)在[0,+∞)上可导，f(0)=1且满足等式：`f'(x)+f(x)-1/(x+1) \int ^x_0 f(t)dt=0`#求f'(x)。
...
题9：设$f(lnx)=\frac{ln(1+x)}{x},计算\int f(x)dx$。<b c=v>犯了dx替换为d(lnx)的思想错误！</b>
>解析：令t=lnx，则x=e^t，则：$\int ln(1+e^t)/e^tdt$计算不定积分。

题10：计算1. $\int ^{\pi}_0f(x)dx=0,\int ^{\pi}_0 f(x)cosx dx=0,求(0,\pi)内有f(a_1)=f(a_2)=0$
...
>解析1：$x的偏导:F'_x=\frac{∂F}{∂x}=\frac{ysinxy}{1+x^2y^2}$#被积函数直接是F的导，所以直接使用。
...
题4：设p=p(x)是抛物线$y=\sqrt{x}$上任一点M(x,y)(x>=1)处的曲率半径，s=s(x)是该抛物线上介于点A(1,1)与M之间的弧长，计算$3p=\frac{d^2p}{ds^2}-(\frac{dp}{ds})$的值。
