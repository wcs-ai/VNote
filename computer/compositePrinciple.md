...
**主存地址映射**：如果是要求设计主存地址，按下面右侧的位数计算各部分所用位数即可。
- **直接映射**：每个主存块号只与1个缓存块号对应。
>主存块号对应的缓存块号：`i= j（主存块号）mod c（缓存块数）`#导致每个缓存块号固定与哪几个主存块对应，不够灵活。
>主存地址字段组成：<b c=v>=主存字块标记 | 缓存字块地址 | 字块内地址</b>；
>`主存字块标记=主存字节地址-cache字节地址`#<b c=gn>字块标记位数=主存字节数位数表示-cache字节数位数表示</b>
- **全相联映射**：主存中每一个字块映射到cache的任何一个位置。但所需逻辑电路较多，成本较高。
>主存地址字段组成：<b c=b>=主存字块标记 | 字块内地址</b>;
>`主存字块标记=主存字节地址-块内字节地址`#<b c=gn>字块标记位数=主存字节数位数表示-块内字节数位数表示</b>
- **组相联映射**：上面两种映射方法的折中，将cache划分为Q组，每组有R个块。
>缓存的组号：`i= j（主存的块号）mod Q`#mod即为模运算（取余）。
>主存地址字段组成：<b c=b>=主存字块标记 |  组地址 | 字块内地址</b>;
>`主存字块标记=主存字节地址-组的表示-块内字节地址`#<b c=gn>字块标记位数=主存字节数位数表示-组数位数表示-块内字节数位数表示</b>
...
<img src="vx_images/498204231153.png" style="height:260px;"/>     <img src="vx_images/130305249579.png" style="height:260px;"/>   <img src="vx_images/1891806237446.png" style="height:260px;"/>
