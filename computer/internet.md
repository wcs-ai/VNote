...
- TCP连接释放：<b c=gn>握手及释放过程如下图</b>。注意各阶段状态名[^a4]
（1）释放过程：`A-(FIN=1,seq=u)->B-(ACK=1,seq=v,ack=u+1)->A`#A会先向B发送FIN=1的释放报文，然后A释放连接，此时A处于半关闭状态（`A能接受B的数据`，但不能再发送）`TIME-WAIT`状态
（2）然后B收到后发出确认关闭报文，B再释放链接，A收到后彻底进入关闭状态。
![tcp](vx_images/2777757131248.png =372x)                                ![tcp-struct](vx_images/3317604211248.png =525x)
...
（3）400 Bad Request 服务器无法理解请求的格式，客户端不应当尝试再次使用相同的内容发起请求。401 Unauthorized 请求未授权。403 Forbidden 禁止访问。404 Not Found 找不到如何与 URI 相匹配的资源（4）`500` Internal Server Error 最常见的服务器端错误。`503`Service Unavailable 服务器端暂时无法处理请求（可能是过载或维护）。
...
# 二、习题：
- [真题1](https://wenku.baidu.com/view/ba9b1539f41fb7360b4c2e3f5727a5e9846a2779.html)
...
>解析：有噪声情况，计算公式与上面不一样。信噪比$S/N=10\log_{10}[S/N]=30dB=>S/N=10^3$，<b c=r>注意是求S/N，而不是信噪比S/N</b>
...
><b c=gn>注：序号值是标出该报文段第一个自己的编号，最大序号255则表示可发送255个这样的报文段（序号用完前）</b>
><b c=v>注：255*64=16320B<2^16^-1（ip数据报最大长度）</b>所以其255个报文段都可以在一个ip报发送，即1个生存时间内！
>解析：8b即为8位，最大序号为255，因为序号使**用完后又会从第一个序号开始**编号使用，<b c=gy>但同一序号不能再同一网络传输中使用，所以每次最多传255个报文段。</b>
>`最高数据率=255*64*8/30=4352 b/s`#可将除数部分的1024换成1000，便于计算。
...
>解析：因为给出了带宽，所以要计算发送时延。<b c=gn>未说加首部，停止等待协议的情况，最好别加</b>[^a4]
...
题5.5：若大小为12B的应用层数据通过1个UDP和1个TCP，则UDP数据报和TCP段实现的有效载荷(应用层数据)最大传输效率？
>解析：数据量比上它们运输层协议的字节总长即可，TCP：12/(12+20)，UDP：12/(12+8)
...
[^a3]: 二进制位数与其2的指数问题：计算位数n所能包含的个数时（排列问题）直接是2^n^，计算位数n能表示的最大数是2^n^-1个；
如：11表示3=2^2^-1,两位二进制含有`00,01,10,11`共4（2^2^）种情况。所以要看计算于哪种情况。
[^a4]: 弱点：用了该项角标的点均需重点记忆；