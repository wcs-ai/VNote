...
![ljieb](_v_images/20210406230847202_18605.png =400x)            ![](vx_images/39440211162.png =510x)
```c++
// 创建邻接表，存储无向图
...
法
    ...
    - **存储有向图时**：顶点的`firstarc`指向其第一条入度边。【如上右图】
    - **逆邻接表**：（反映顶点出度的情况）指存储有向图时，`firstarc`指向第一条出度边。
