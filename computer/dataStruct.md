...
（8）**插入**：在第 i 个位置插入（共 n 个元素）则元素移动次数为：`n-i+1`（注意这里描述的是==第 i==个，不是指索引）
...
mind：前中后序遍历得到的叶子节点的“==先后顺序==”总**是一样的**；==前序与后序不能唯一确定1颗二叉树==（不知道各子树上的根节点）
...
- 二叉树转森林：<b c=b>只有右节点的，都连接到其爷爷节点上。</b>
  ...
  <img src="_v_images/4475039221052.jpg" height="200px"/><img src="_v_images/3676640239478.jpg" width="500"/>
...
- **逆邻接表**：指存储有向图时，`firstarc`指向第一条==入度边==。
  ...
  >
  > <img src="_v_images/20210406230847202_18605.png" style="height:260px"/><img src="vx_images/39440211162.png" style="height:260px;"/>
...
>
> <img src="vx_images/5276251231161.png" style="height:240px;"/> <img src="vx_images/1973411019588.png" style="height:240px;"/>
...
**例**：关键字{18,2,10,6,78,56,45,50,110,8},装填因子 a=0.77，散列函数`H(key)=key MOD 11`用线性探测法解决冲突，构造散列表！
...
**实现**：
（1）创建指向数组头尾的两个指针 i,j，指定 1 个基数 X；(==初始基准使用下标 0 处的值==)
（2）用 j 从==右向左扫描==，遇到==比 x 小==的，则与 i 所指位置的数**交换**，i+1;（注意是==先从右开始==）
（3）用==i 从左向右扫描==，遇到==比 x 大等于==的，则与 j 当前所指位置的数**交换**,j-1；
（4）交替重复(2),(3)知道 i，j 指向同一位置时，此即为基数 X 的位置，**完成第 1 趟排序**；
（5）重复(1)-(5)n 次，完成排序；
...
堆排序就是利用**堆**[^dui].这种数据结构建立的一种排序。属于树形排序，也是选择型排序（这里以大根堆为例，小根堆类似）

- (1)建立大根堆：从 1 个元素开始检测（`for(i=n/2;i>=1;--i){//调整堆代码}`循环调整 n/2 次），每个节点要比其左右孩子大，若不满足则将父节点==与其最大的子节点交换位置==；设最终排序结果集为 T；
- (2)再调整堆：将堆顶元素`a`与最后一个元素 b**交换位置**（==完成 1 次排序==，不增加额外空间的做法），使用（1）方法将剩余的元素重新调整为大根堆。
- (3)重复步骤（2），直到 F 中放满所有元素，完成排序
...
例：有待排序的数组 a 和一个空数组 b(与a==相同长度==)，扫描 a ，确定其每个元素，小于其值的==个数作为索引==index放到 b 中对应下标位置处（如a[0]=12,a中小于12的数有5个，那么b[5]=12）
...
桶排序是计数排序的升级版，求出带排数组的最大最小值确定好桶数(空数组数)，分段的将其分到这些空数组中(例：<50 的放到第 1 个空数组,>=50&&<110 的放到第二个空数组...)各子数组使用其它排序算法或继续使用桶排序方法排好序，再按顺序将这些排好序的子数组拼接起来。

### 9、基数排序

是一种用多个关键字来排序的方法（如牌的花色和面值共同决定牌的大小）有两种排序方法如下：
**最高位优先法**：先按不同 “花色” 分成 有次序的 4 堆， 每堆再按面值大小整理有序（全部是数值时可按数值范围分桶，再每桶排序）
**最低位优先法**：全体关键字参与。
**例**：对 278，109，063，930，589，184，505，269，008，083 进行从小到大的基数排序：
思想：关键字都是数值，所以==设置 10 个桶==，先以“个位”为关键字，方如对应序号的桶内，再从桶内拿出（每个桶`后->前的顺序`）完成 1 次排序

- 按个位为关键字排序，放入桶（注意==多个数在同 1 桶中时，它们的顺序==）

| 桶 0  | 桶 1  | 桶 2  |  桶 3   | 桶 4  | 桶 5  | 桶 6  | 桶 7  |  桶 8   |    桶 9     |
| :---: | :---: | :---: | :-----: | :---: | :---: | :---: | :---: | :-----: | :---------: |
|  930  |       |       | 083/063 |  184  |  505  |       |       | 008/278 | 269/589/109 |

- 从桶中拿出：930,063,083,184,505,278,008,269,589,109（注意每个桶多个数时拿出的顺序，==与原始序列一致==）
- 在第 1 趟排序结果上，再以“十位”为关键字排序，放入对应序号桶中，如此重复到对所有关键字都进行过排序

### 10、归并排序

（1）1,2 进行对比排序，3,4 进行对比排序...（2）1,2 为一组，3,4 为一组，这两组进行排序（内整体再排序），依次如此...

> **特点**：稳定、记录数大

### 11、外部排序

数据太多，无法全部放在内存时，数据存储在磁盘，使用内存空间辅助排序。常用的是归并排序（不需要将全部记录读入内存）

1、归并排序：k>2时可称多路归并排序（每次取1个段首最小值，1个组的数据全部变为1个长的有序序列）
（1）将n个记录分为m个**记录段**（每个记录段都能放入内存）
（2）再每k个**段**分为1组，同1组中，各段段首读入内存
（3）用某种算法选出各组中，段首最小的值；将这些最小值写回外存（==单独存储==）
（4）依次重复以上步骤，取各段首最小值，最后这个组的数据全部导出为1个较长的有序记录段；
（5）其它组也同样操作，得到的所有排序记录合并完成1趟排序；再从（1）开始，直到排序完成

2、置换选择排序：高效且**不受内存空间限制**（如当归并排序中划分的记录段任然无法读入内存时）

3、最佳归并排序：归并排序中每趟归并操作需要两次I/o操作，最佳归并排序则是**减少归并次数**的方法。

4、败者树：归并中有1个很**多次出现的操作**是从多个值中选出最值，败者树则是**提高选择最值效率**。
...
- 特性集：
  （1）**交换类和选择类**排序，每次排序结果都==至少会有 1 个元素放在最终位置==
...
**定义**：解决问题的步骤序列。（注意是问算法的特性，还是效率）
**算法设计过程**：理解问题、确定数据结构、设计算法、正确性证明、分析算法、编写代码

## 0、算法分析

- 算法的 5 个特性：有穷性、确定性、可行性、有零个或多个输入、有一个或多个输出。**算法的可行性**是指指令不能有二义性。
- 算法+数据结构=程序
- 复杂度依赖于 3 方面：问题规模，算法的输入、算法本身；
...
（1）O记号表示：设有非负函数f(n) 和 g(n)，存在常数C和n0，当n>=n0时，有`f(n) <= Cg(n)`，则记为`f(n) = O(g(n))`
（2）大Ω记号表示：设有非负函数f(n) 和 g(n)，存在常数C和n0，当n>=n0时，有`f(n) >= Cg(n)`，则记为`f(n) = Ω(g(n))`
（3）大θ记号表示：设有非负函数f(n) 和 g(n)，存在常数C1、C2和n0，当n>=n0时， 有 `C1g(n) <= f(n) <= C2g(n)`，则记为`f(n) = θ(g(n))`
...
（4）时间复杂度还可能呈现的形式有：O(1)、O(n)、O(n^2)、O(log(n))、O(2^n)#分别是常量阶、线性阶、平方阶、对数阶、指数阶。（赋值、计算这些不用计算在时间复杂度之内）
即使有了这些方法，计算时间复杂度依然不是那么准确的，比如算法很复杂的情况，各编程语言中调用的内置函数的时间。
（5）相同规模 n 下复杂度 O(n)的算法，时间上总是优于复杂度`O(2^n)`的算法。
（6）同一算法语言实现的级别越高，效率越低。

1.2、**主方法**：算法多数情况由n项组成$T(n)=3f(n)+g(n/2)+1$,此时我们==取随n递增最大的1项，去其系数做为算法的时间复杂度==
（1）如：f(n)=an+b取，出式子中随 n 增大而增大最快的项，然后把其==系数变为 1==；
（2）若 f(n)是一个常数，则得到`O(1)`；

> 如 1：f(n)=1000，则得到：O(1)
> 如 2：$f(n)=2n^3+4n^2+7$,则最大项为$2n^3,得O(n^3)$
> 如 3：$f(n)=15+2^n+n^2$的渐进性态为$2^n$

2、**空间复杂度**：比较常用的有：O(1)、O(n)、O(n²)
（1）如果算法执行所需要的**临时空间**==不随变量 n 的大小而变化==，即此算法空间复杂度为一个常量 O(1)
（2）即输入==数据本身所占的空间，创建的临时变量==，不计入空间复杂度；
（3）如`int[] m = new int[n]`,这个数据占用的大小为 n，因此空间复杂度为 O(n);==在循环中时再乘循环的次数==

3、递归复杂度：

## 0.1、正确性证明

正确性证明包括两个方面：①证明关于输入与输出之关系的命题是正确的；②证明算法中的公式及计算方法是正确的。“正确”的含义包括以下三个方面：
...
**P问题**：(Polynominal)，对于输入规模n，可以在$O(n^k)$,（k是有限值）这种多项式时间内解决的问题称为**较易处理的问题**；

**NP问题**：(Non-deterministic Polynominal)，一般指难以在多项式时间内求解的问题（但并未有人证明它的复杂度是否），多项式复杂程度的非确定性问题

**NPC问题**：(NP Complete)只有把解域里的所有可能穷举之后才能得出答案，这是 NP 里面最难的

NP完全性证明：证明一个问题是否属于NP类问题；使用规约，将A的问题规约为B？？？

## 1、分治与递归

- 可用分治法求解的问题：快速排序、选择排序、归并排序、棋谱覆盖问题、实现循环赛日程表、Strassen 矩阵乘法、大整数乘法
- 基本思想：将 1 个规模 n 的问题分解为 k 个规模较小的子问题、==子问题相互独立且与原问题相同==。
- **求两数最大公因子**：使用辗转相除法
...
- **实现组合函数**：从集合{1,2,..,n}中选择 k 个元素的所有组合
...
- 基本思想：将待求问题分解成若干子问题、先求解子问题，然后从这些子问题的解得到原问题的解。
- 算法基本步骤：（1）找出最优解的性质，并刻划其结构特征。（2）递归地定义最优值。（3）以自底向上的方式计算出最优值。（4）根据计算最优值时得到的信息，构造最优解。
- 基本要素：最优子结构性质与重叠子问题性质；（通常以==自底向上==的方式求最优解）
- 用到动态规划的算法：最长公共子序列、矩阵连乘问题、应用 johnson 法则的流水作业调度、背包问题
- **最长公共子序列**：（LCS）是一个在一个序列集合中（通常为两个序列）用来查找所有序列中最长子序列的问题。
- **示例描述**：假设问题为从`S→A→B→C→E`，S 有{s1,..,s5},A 有{A1,..,A5},..要选出一条最好的路径
  （1）从 E 中所有可能的结果开始，往后推（==自底向下构建的性质==）
  （2）选出在在 E1 的前提下，上一条最好的路径 C2，E3 的前提下，上一条最好的路径 C5，...
  （3）选出在`C2→E1`前提下（体现==子问题间是有关的==），上 1 条最好的路径 B4，...
  （4）依次如此，选到 S 集合时截止，E 中选出了几个节点，就有几条路径，==每条路径计算它们的权重==，相比得到最优的 1 条即是解
...
- 贪心与动态规划的共同点：最优子结构性质
- 贪心与动态规划主要区别：贪心选择性质
- 可用贪心法解决的问题：单源路径最短问题、最小花费生成树问题、背包问题
- **背包问题**：给定一组物品，每种物品都有自己的重量和价格，在限定的总重量内，我们如何选择，才能使得物品的总价格最高
  贪心求背包问题思想：（1）依次计算每种物品单位重量的价值（`p[i]=v[i]/w[i]`）（2）依贪心选择策略,尽量将 p 中值大的物品放入背包，直到装满为止。

```c++
void Knapsack(int n,float M,float v[],float w[],float x[]){
	Sort(n,v,w);
	int i;float c=M;
	for(i=1,i<=n;i++){x[i]=0;} // 初始化x
	for(i=1,i<=n;i++){
		if(w[i]>c) break;
		x[i] = 1; c-= w[i]; // 不大于最大值的则记录为1
	}
	if(i<=n) x[i]=c/w[i];
}
```

- **活动安排问题**：知道各个活动的起始与终止时间，在规定起始与终止时间内，选择举办的活动顺序，尽量举办多个活动

- **装载问题**：n 个集装箱要装上 C 船，集装箱 i 的重量为`w[i]`,且重量之和小于 C 可承载重量，找 1 个合理的方案，装载尽量多的箱子。
  解思想：每次从剩下的货箱中，选择重量最小的（==排序后依次选择==）
  
- **贪心与动态规划的区别**：

  共同点：都需要最优子结构性质；

  不同点：（1）动态规划：每一步选择1个依赖子问题的解，规划条件是子问题的重叠性质，自底向上求解；

  （2）贪心：每一步选择1个不依赖子问题的解，规划条件是贪心选择性质，自顶向下求解；

## 4、回溯

- 回溯法：是指具有限界函数的的深度优先搜索算法。回溯法是一种即带有==系统性==，又带有==跳跃性==的搜索算法。
- **步骤**：
  （1）定义解空间，它至少包含问题的 1 个（最优）解。
  （2）确定易于搜索的解空间结构，使得能用回溯法方便的搜索整个空间。
  （3）以深度优先的方式搜索解空间，且在搜索过程中使用剪枝函数避免无效搜索。
  （4）满足条件则继续向下搜索；
  （5）不满足则回溯到上 1 个节点处，重新选择；
- **子集树**：当问题需要 n 个元素的 1 个子集来优化函数时，解空间树称为==子集树结构==（如回溯法解 0/1 背包问题时）
- **排列树**：当问题需要 n 个元素的一个排列来优化函数时，解空间树称为==排列树==（如回溯法解批处理作业调度问题/旅行售货员问题时）
- **界定函数**：用来杀死活动节点的策略
- 回溯法算法框架按照问题的解空间一般分为：子集树算法框架、排列树算法框架。
- 显著特征：搜索过程中动态产生问题的解空间，在任何时刻，算法==只保存从根节点到当前节点的路径==，如果解空间树中从根节点到叶节点的最长路径长度为 h(n)，则回溯法所需的计算空间通常为`O(h(n))`
- 在问题的解空间中，按深度优先策略，从根节点出发搜索解空间树
- 剪枝函数：为避免无效搜索采取的策略
- 回溯法效率与问题的==解空间形式无关==
- **回溯法求八皇后问题**：在 8×8 格的国际象棋上摆放八个皇后，使其不能互相攻击，即任意两个皇后都不能处于同一行、同一列或同一斜线上，问有多少种摆法？
  其解空间树为==排列树==
...
#include<iostream>
using namespace std;
// 其中vis[0][i]表示列，vis[1][i]和vis[2][i]表示对角线
int vis[3][15],tot;
...
- 回溯求解 0/1 背包问题：`n=4,c=7,p=[9,10,7,4],w=[3,5,2,1]`（c 为可承受的最大 1 重量,p 存储价格,w 存储重量）对象的收益密度为`[3,2,3,5,4]`
...
double weightOfCurrentPacking, profitFromCurrentPacking, maxProfitSoFar;
...
- theCapacity：背包容量
*/
double knapsack(double *theProfit,double *theWeight,int theNumberOfObjects,double theCapacity){
	//初始化全局变量
	...
	maxProfitSoFar = 0.0;
	// 定义1个数组，存储收益
	element* q = new element[theNumberOfObjects];
...
// 排序函数
void mergeSort(element* q,int nfb){}

void rKnap(int currentLevel)
	// 到达一个叶节点
	if (currentLevel > numberOfObjects) {
		maxProfitSoFar = profitFormCurrentPacking
		return;
	}
	...
	if (weightOfCurrentPacking + weight[currentLevel] <= capacity) {
		...
		profitFormCurrentPacking += profit[currentLevel];
		// 递归
...
- 常见的两种分支限界法：队列式（FIFO）与优先队列式
- 分支限界法在问题的解空间中按==广度优先==策略搜索
- 可解决的问题：单源最短路径问题
- 分支限界法设计算法的步骤：
  （1）针对所给问题，定义问题的解空间（对解进行编码）
  （2）确定易于搜索的解空间结构（按树/图组织解）
  （3）以广度优先或以最小耗费（最大效益）优先的方式搜索解空间，并在搜索过程中用剪枝函数避免无效搜索
...
- 入队操作：对栈 s1 进行入栈操作；
- 出队操作：将栈 s1 中的元素全部出栈，再全部入栈 s2，然后 s2 出栈 1 个元素；
...
- **递归程序**：求$F(m)=\begin{cases}1 &,0<m<=1\\ mF(m DIV 2),&m>0 (DIV为整除)\end{cases}$
...
```c++
void nonrecure(int m){
	int stack[5] = {0,0,0,0,0}; // 自定义栈
	int top = -1; // 栈指针
	// 入栈操作
	void shedIn(int v){
		if(top>=sizeof(stack)) std::cout<< '栈满';
		stack[top] = v;
		++top;
	}
	// 出栈操作
	int shedOut(){
		if(top==-1) return -999; //栈空
		int v = stack[top];
		--top;
		return v;
	}
...
**解思路**：使用==双指针==（线性表结构常用的方法）
设两个指针 i(指向第 1 个元素)，j(指向最后 1 个元素)；一边让 i，j 所指数据交换，一边相向而行，直到遇到为止
...
问题：将线性表中的元素循环左移p个元素；
思路：将前p个元素逆置，再将后面的元素逆置，最后再整个逆置即可；
...
问题：两个线性表都是升序排列，将它们合并后依然是升序结构，不使用额外的空间（链接存储时）

```c++
// 顺序结构时的实现
SeqList merge(SeqList L1,SeqList L2){
    SeqList L; // 新建1个列表存储结果
    L.length = L1.length+L2.length;
    int i=0,j=0; // L1,L2的指针
    int k=0; // 已归并好的元素个数
    while(i<L1.length || j<L2.length){
        if(j>=L2.length || i<L1.length && L1.data[i] <= L2.data[j]){
            L.data[k++]=L1.data[i++];
        }else{
            L.data[k++]=L2.data[j++];
        }
...
从集合{1,2,..,n}中选择 k 个元素的所有组合;排列函数实现与此类似，且更简单
...
function Cmn(data, k) {
  ...
  let groupList = [];
	 ...
	 * num: 拼接的层级
	*/
  function comb(lastStr, i, j, num) {
    //debugger;
    let levels = [];
    // 拼接可能的组合
    for (let si = i, t; si <= j; si++) {
      t = lastStr + G + String(si);
      levels.push(t);
      // 继续找到si下面的所有可能组合
      if (num < k) {
        comb(t, si + 1, j, num + 1);
  ...
  comb("", 0, n - 1, 0);
  /****将下标转成具体值*******
	 * 答题时可以，可以将上面的拼接写成打印输出
	 * 然后省略这里的循环查找
	*/
  let resList = [];
  for (let si = 0, temp, arr, len2 = groupList.length; si < len2; si++) {
    temp = groupList[si].substring(1); //去除最前面的 -符
    arr = temp.split(G);
    arr.map((v) => {
      resList.push(data[v]);
    });
...
思路：（1）对二叉树进行层次遍历；（2）放入堆结构（非大/小根堆）（3）若中间出现**缺失的空位**或**总节点数**不在$(2^{n-1}-1,2^n - 1)$内则不是，则不是完全二叉树

## 8、图问题

（1）**求图的最大连通分量**：

思路：使用深度优先遍历，递归过程中多加1个参数保存经过的节点数；检测当前节点值是否在列表（放到递归参数中）中有出现过；

（2）**求有向图的根**

根：能到达其它所有点的节点；
思路：先查找无前驱的节点，多余1个直接结束；否则每个节点深度优先遍历，并存储起来，最后列表中个数与总节点数一致，则是根节点；

（3）**建医院问题**：有向，无向图，都是要以下方法

问题：n个村庄中，有路连接的村庄，它们可以互通，问在哪个村庄建个医院可以最小费用？
**思路**：（1）用佛洛依德求各点到其它点的最短路径矩阵；（2）计算医院**建在各点时**，==其它点到该点的代价总和==；
（3）比较各点的代价总和，选择**代价总和最小的那个点**，在此建立医院；

（4）**总费用问题**：有向，无向图，都用以下方法

问题：几个村庄构成1个图结构，在这些村庄假设通讯网，让其全部连通，求总费用最小
思路：使用==最小生成树==，所得树结构，就是方案；

## 11、有向图2元表达式问题

存储表示：

求后缀表达式：建立算术表达式的邻接表，再转为逆邻接表，进行拓扑排序得到后缀表达式

## 12、交替冒泡排序

**思路**：1、普通冒泡的基础上改造；
2、第一次上浮冒泡结束后再进行1次下沉；
3、即每一轮有1个最大值上浮到末尾，有1个最小值下沉到开头；

```js
/*****交替冒泡******/
function alternatePop(data){
  let change = 1,low = 0,high = data.length-1;
  while(low<high && change){
    change = 0;
    // 大值上浮
    for(let i=low,temp;i<high;i++){
      if(data[i]>data[i+1]){
        temp = data[i];
        data[i] = data[i+1];
        data[i+1] = temp;
...
思路：以k作为基值，进行1趟快速排序即可		

政治新增：
1、中国人权发展最显要的特征：**人民性**、最大人权（人民幸福）、基本人权（生存、发展）
2、职业道德，志愿精神的精髓：奉献
3、三元里斗争：第一次大规模的反侵略斗争
4、义和团运动：粉碎帝国主义列强瓜分中国中发挥**重大历史作用**
5、新民主主义革命时期，对中国革命有决 定意义的新起点是：**从大城市进军转向农村**
6、新时代爱国统一战线的历史责任：**促进中华儿女大团结**
7、构建新发展格局的**关键在于**：经济循环的畅通无阻
	**特征是**：实现高水平的自立自强

8、《二十大报告》社会保障中新增的内容：**安全规范**（其它几条为：覆盖全民、统筹全乡、公平统一）
9、《二十大报告》共产党使命：以中国式现代化推进中华民族伟大复兴
		建设社会主义现代化国家的**首要任务**：高质量发展
		加快实施生态系统保护重点：国家重点生态功能区、自然保护地、完善生态保护补偿制度

10、冬奥会理念中，放在首位的是：**绿色办奥**、冬奥会主题：向东；胸怀大局指：心系祖国，志存高远
10.1、一次分配靠市场、二次分配靠政府、三次分配靠慈善
10.2、新经济政策特征：发展商品经济，重点是：放在实体经济；经济发展**指导原则**：坚持新发展理念
10.3、乡村振兴关键：产业振兴
10.4、民主政治本质：全过程人民民主
开万国之旗：走自己的道路
...
- 中国共产党在革命性锻造中更加坚强有力；
- 中国人民的前进动力更加强大、奋斗精神更加高；
- 实现中华民族**不可逆转的历史进程**；
- 科学社会主义21世纪的中国焕发**新的蓬勃生机**；

12、两个确立意义：

- “两个确立”是新时代实现伟大变革关键所在，是**党十八大以来最重要政治成果**；
- 是新征程上赢得**未来的必然要求和根本保证**。拥有马克思主义科学理论指导是我们党信仰信念，**把握历史主动的根本所在**；

13、神舟15号发射成功意味着：

- 我国立项实施以来的第二十七次飞行任务
- 进入空间站载人阶段的第4次载人飞行任务
- 标志着空间站**关键技术验证和建造阶段规划**的12次发射任务全部圆满完成

14、社会再生产核心问题：社会总产品的实现问题
16、决定两岸关系，实现祖国**完全统一的关键**：国家的发展进步；两岸关系**和平发展的关键**：九二共识
17、延安精神精髓：坚定正确的政治方向
18、《共迎时代挑战，共建美好未来》内容：推动更加包容、更加普惠、更有韧性的全球发展
19、国家安全：民族复兴的根基;
		   国家安全观的根本：**政治安全**
