...
- 数组的常见操作：查找和修改
- **压缩存储**：对**对称、稀疏这类特殊矩阵可进行**。方法一般有==3元组和十字链表==
	**三元组压缩法**：表示出数组中非 0 位置的值：`((1,5,1),(3,2,-1),(4,5,-2),...)`序号从 1 开始，前两个值为行号和列号，第 3 个值为存储值。
	**顺序存储表示**时将这些当行写在表里，然后最上面加上`行数|列数|数值个数`。
...
5. 平衡二叉树：为了避免二叉搜索树变成“链表”，我们引入了平衡二叉树，即让树的结构看起来尽量“均匀”，左右子树的节点数尽量一样多。
- 又称 AVL 树，指的是左子树上的所有节点的值都比根节点的值小，而右子树上的所有节点的值都比根节点的值大，且左子树与右子树的高度差最大为 1。
- **平衡因子**：一个节点的平衡因子为：左子树高度-右子树高度
- 构造：先按照生成二叉搜索树的方法构造二叉树，直至二叉树变得不平衡，即出现这样的节点：左子树与右子树的高度差大于 1。
- 调整：主要有四种调整方式：LL（左旋）、RR（右旋）、LR（先左旋再右旋）、RL（先右旋再左旋）。==一般按从左到右由小到大顺序排序即可==
...
- 冲突处理方法：
  ...
  3. 二次/平方探测法：如在d=5处冲突，则检测：$d+1^2,d-1^2,d+2^2,d-2^2,..$处索引位置，直到有空位。
...
9. **索引存储**：分别存放数据元素和元素间关系的存储方式。所有的存储结点存放在一个区域。另设置一个**索引区域**存储结点之间的关系。
...
4. **希尔排序**：(==不属于选择和交换类==)（1）取增量 `d1<n`，所有序号间隔为 d1 的分为一组（如 d1=5，序号 0,5 就是一组），各组中分中别直接进行插入排序(如序号 5 的数和 0 的数交换)。（2）去增量 d2 再分组，各组进行直接插入排序。（3）取到 dt=1 时即为所有元素放一起进行一次直接插入排序即可。
...
**实现**：
（1）创建指向数组头尾的两个指针i,j，指定1个基数X；
（2）用j从右向左扫描，遇到比x小的，则与i所指位置的数交换;
（3）用i从左向右扫描，遇到比x大的，则与j当前所指位置的数交换；
...
- **筛选法步骤**：假设 D 已经是一个有序的小根堆：`D=[5,8,12,19,28,20,15,22]`#现插入一个元素 3（末尾插入）为满足小根堆需求必须调整堆。
  （1）插入 3 后，3 所在索引为 8。算出其父节点索引：`K1=(8-2)/2=3`，再计算 K1 的父节点：`K2=(3-1)/2=1`，K2 父节点索引 0。**索引从 0 开始，奇数-1，偶数-2**
  （2）若`D[0]>D[8]`，则交换。若`D[3]>D[8]`则交换，若`D[1]>D[3]`则交换。若`D[0]>D[1]`则交换。
  （a）**建立堆**：也是使用（1）（2）完成。<b c=b>删除大/x 小根堆，堆顶元素后，把堆顶元素当做无穷小/大看待，然后使用筛选法调整</b>
- 特点：是不稳定排序、只能用于顺序结构，不能用于链式结构 、记录较少时不建议使用，时间复杂度：$O(nlog_2 n)$（==与初始记录是否有序无关==）。
...
8. **基数排序**：是一种用多个关键字来排序的方法（如牌的花色和面值共同决定牌的大小）有两种排序方法如下：
...
**比较图表**：
| 排序方法     | 平均时间性能     | 最好时间性能     | 最坏时间性能     | 辅助存储空间 | 稳定性 | 类型 |
| :----------- | :--------------- | :--------------- | :--------------- | :----------- | :----- | :--- |
| 直接插入排序 | $O(n^2)$         | $O(n)$           | $O(n^2)$         | $O(1)$       | 稳定   | 选择 |
| 冒泡排序     | $O(n^2)$         | $O(n)$           | $O(n^2)$         | $O(1)$       | 稳定   | 交换 |
| 简单选择排序 | $O(n^2)$         | $O(n)$           | $O(n^2)$         | $O(1)$       | 不稳定 | 选择 |
| 快速排序     | $O(nlog_2n)$     | $O(nlog_2n)$     | $O(nlog_2n)$     | $O(log_2n)$  | 不稳定 | 选择 |
| 堆排序       | $O(nlog_2n)$     | $O(nlog_2n)$     | $O(nlog_2n)$     | $O(1)$       | 不稳定 | 交换 |
| 归并排序     | $O(nlog_2n)$     | $O(nlog_2n)$     | $O(nlog_2n)$     | $O(n)$       | 稳定   | 交换 |
| 基数排序     | $O(d(n+raddix))$ | $O(d(n+raddix))$ | $O(d(n+raddix))$ | O(radix)     | 稳定   | 选择 |

1.  特性集：
  （1）**交换类和选择类**排序，每次排序结果都==至少会有1个元素放在最终位置==
	（2）只选出前几个元素的排序结果时，堆排序最快
