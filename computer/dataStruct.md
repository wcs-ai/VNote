...
- **中序序列个数**：中序序列为1,2,..,n的二叉树，其二叉树情况可能有$\frac{1}{n+1}\frac{2n!}{n!*n!}$种。
- <b c=b>度为0的节点个数总是比度为2的节点个数多1</b>
...
- 一般使用二叉树长期存储表达式，每个节点是数字或字符，一般存为后序遍历得到后缀表达式。中序遍历得到中最表达式
- 计算树中存储的二元表达式：
```c++
int calc(cNode p){
    ...
    int lv,rv;    //
    if(C){
        lv = calc(p->left);rv = calc(p->right);                        //递归计算左右
        if(!p->left && !p->right) p->val = p->data - '0';              //左右节点为空的情况。将数字字符转换为整数存储
        else{
            switch(p->data){
                case "+":p->val = p->left->val + p->right->val;break;  //左节点的值+右节点的值
                case "-":p->val = p->left->val - p->right->val;break;
                case "*":p->val = p->left->val * p->right->val;break;
                case "/":p->val = p->left->val / p->right->val;break;
            }
        }
