...
![yebiao](vx_images/1271033150952.png =359x)                                ![duanb](vx_images/448034169378.png =480x)
...
2. **对换性**：如果一个进程占用大量的内存，但由于某个原因阻塞了，而其它进程又因为内存不足无法运行，这是极其糟糕的事，所以有了对换，可以**将阻塞的进程先切换到外存等待**。
...
- **物理结构**：指逻辑结构在物理存储空间中存放方法和组织关系。组织关系如下：
...
>**索引结构**：文件中必须使用专门的存储空间存放索引表，读取该类文件时也要读取索引表，才能知道相应逻辑文件在外存块上的位置。`缺`：增加了索引表空间的开销，大型文件的索引表甚至可能超出文件本身大小。
...
记录寻址：为访问顺序文件中的一条记录，要找到该记录的地址，找记录地址的方法有：隐时寻址和显式寻址方法。
...
（2）根据找到的二进制号，据位示图转为行i，列j表示，盘块号`b=n(i-1)+j`#n为每行为数，一般与机器字长相同。
...
对应关系：
![3种体系结构对应关系](vx_images/3933238140960.png =464x)
...
- 统计TDM：按需要动态的为用户分配，需要解决好传输中的同步和成帧问题。
...
**两个设备**：
1. **普通交换机**：在vlan中作用：实现VLAN划分，交换VLAN成员信息、VLAN中进行数据帧的交换。
- **存储转发方式**：
...
![拥塞控制](vx_images/664243161043.png =463x)                                                                                ![多种拥塞控制法变化](vx_images/1330217189469.png =521x)
