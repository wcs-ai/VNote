...
**进制转换**：[参考学习地址](https://zhuanlan.zhihu.com/p/75291280)
- 十进制转二进制：（1）整数十进制转二进制：将整数除2，得到的余数为2进制位数，然后将商再除二，如此循环直到商除尽，得到的**所有余数组合即为其二进制**。（2）小数十进制转二进制：使用乘2取整法，先将整数部分转为二进制，再小数部分乘2，取整部分，再取整数部分，...最后结果不再小于1时停止。<b c=r>注意将结果反转后是最后的转化结果</b>
