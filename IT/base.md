...
- 二进制转十进制：从右往左每位乘以对应的2的次数，然后相加。例如：00101010 >>`0*2^7+0*2^6+1*2^5+...+0*2^0`>>42，有小数位的话小数部分从左到右开始。<b c=r>注意最后一位是2^0次而不是2^1</b>
...
- 反码：通常是用来由原码求补码或者由补码求原码的过渡码。<b c=v>正数的反码与原码相同，负数的反码将原码取反，1变为0，0变为1</b>
- 补码：是计算机把减法运算转化为加法运算的关键编码。<b c=v>正数的补码与原码相同。负数的补码为：原码的反码的最后一位加1</b>
...
**机器字长**：cpu一次能处理数据的位数，因此系统分为32和64位之分。列入x86与x64的区别：(X86就是我们一般用的32位的系统，X64就是64位的系统):
...
**位&字节**：`1byte=8bit=1B,1KB=1024B` 。 <i c="gn">byte即为字节、bit为位，一个字母或单个数值占1个字节，一个汉字一般占两个字节。</i>传输速率用bit/s或者b/s或者bps。
...
(2)、地址总线：存储各数据、io设备位置的总线，如cpu在取数时还需要将数对应的地址传到数据总线上！。地址总线为20根，则存储单元个数为2^20个。
...
**带宽**：计算机网络中用来表示传输数据的能力，**实际情况下**单位时间内网络中某信道所通过的最高数据率。一般bit/s为单位（bps）。
...
**二叉树**：与树一样具有递归性质。每个节点最多有两个子节点，第k层最多有`2^(k-1)`个节点，为k层的树最多有`2^k-1`个节点。 节点有左右之分，是**有序树**。
...
>二叉查找树查找效率与树形有关，当节点呈单枝排列时效率最低。

**哈夫曼树(最优二叉树)**：一个节点到另一个节点的分支称为两节点间的路径，路径上的分支数目称作路径长度，树的路径长度等于根节点到每一个节点的路径长度的总和。而每个节点有权重的情况还要乘以节点权重(如：根节点到a点经过两条路径，a点权重为8，则该条路径长度`WPL=8*2`)，这称为树的带权路径长度。而哈弗曼树是使WPL最小。构建过程如下：
...
**平衡二叉树**：（Balanced Binary Tree）又被称为AVL树（有别于AVL算法），且具有以下性质：它是一 棵空树或它的<b c=gn>左右两个子树的高度差的绝对值不超过1</b>，并且左右两个子树都是一棵平衡二叉树。
