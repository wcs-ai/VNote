...
**二叉树的存储**：对于满二叉树和完全二叉树可以使用顺序存储结构，将各节点编号，依次从上到下，从左到右的存储节点即可。而对于普通树最坏的存储情况是：k层只有k个节点，却需要一个`2^k-1`长的一维数组，因为遇到空的节点要用0或其它占位表示。**用链式存储**：存储每个节点时再存储左右指针域即可，这称为二叉链表，如果再多存一个父节点的指针域则称为三叉链表 。
