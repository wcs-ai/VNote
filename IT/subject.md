...
就称v是数域p上的线性空间。(求是线性空间时要根据这些条件都满足才是)
 2、维数、基与坐标：[线性空间的维数]如果在线性空间v中有n个线性无光的向量,但是没有更多线性无关的向量，那么v就称为是n维的(即任意n+1个向量组合后都是线性相关的)。[基]在n维线性空间v中
n个线性无关的向量e1,e2,...en称为v的一组基；[坐标]设a是v中任一向量，于是a,e1,...,en线性相关，所以a可被这组基线性表出:a=a1*e1+...+an*en，其中系数a1,a2,...an是被向量a和这组基唯一确定的
这组系数就称为a在这组基下的坐标。
...
### 二、概率论知识：
1、伯努利概型：实验E只有两个结果A,A-。而p(A)=p,p(A-)=1-p=q;把E独立重复n次又构成了一个实验(例如抛硬币,一次抛多个改为每次抛一个)称为n重伯努利概型。
2、离散型随机变量：实验E中样本点w出现的次数用e表示mE的总样本空间v在实数域上且为可列可加个，而e=e(w)称为离散型随机变量。
3、期望：[举一个例子]从工厂抽出100支手表,测出每只手表的日走时误差,分布为(支数，日走误差)>>
(3,-2),(10,-1)...[平均日走时误差]=3*-2+10*-1+.../100(总日走误差/总支数)期中可拆分出3/100,10/100
这些是在100支手表中的频率,所以原式可改为s=sum(k*1/p(k)),这个均值称为数学期望。
4、泊松分布，泊松定理：
5、先验分布：在进行一次实验之前我们一般会进行猜测实验结果且我们会多多少少知道一些实验中的一些未知参数可能的值这叫先验知识，我们对该值猜测的值和这样猜测的原因构成的这个分布就是先验分布。这是在实验前进行的所以不影响实验。
6、后验分布：根据样本Xde分布PO及O的先验证分布tt(o)，用概率论中求条件概率分布的方法可算出在已知X=x的条件下o的条件分布，因为这个分布是在抽样后才得到的故称为后验分布。
7、狄利克雷分布：
8、点估计：
9、极大似然估计：极大似然估计是求估计的另一种方法(由高斯提出)，直观想法：假设一个实验会出现若干种情况，若在一次实验中结果A出现则会认为实验条件对A有利，即A出现的概率更大。
假设从容量为n的子样r1,r1,...取到观测值x1,x2,...的概率为：f(r1=x2,...)=p^x1(1-p)^(1-x1)*p^x2...
...
10、概率分布函数：对于离散型数据由于数据比较分散所以在进行实验时很容易记录其分布情况，但对于连续型的数据记录其分布时如果再用离散型的方法却不一定都能有效如：描述在区间[a,b]内随便取一个点落在区间[c,d]的分布。对于其概率可以轻易就计算出f落在[c,d]区间的概率都为p=(d-c)/(b-a)
但若再像离散型变量一样统计分布为:p=x/(b-a)=0，因为取的x是一个点而点的长度为0，这样得出的分布显然是错误的，所以需要重新引入一种方法表示连续型数据的分布。受到在[c,d]区间的概率表示
启发p(c<=v<=d)=p(v<=d)-p(v<=c),右边的表示都为v<=d这种类似的情况所以我们对任意x计算其概率时只要知道p(r(w)<=x)就能计算其概率(上面的v用r(w)替换了，因为这样能表示更多复杂的情况，比如要对随机取的值v做变换之类的，可以把r(w)看成是一种统计规律。这里的x和v都是随机在区间的值，不过两者表示的意义不一样)。F(x)=p(r(w)<=x),引入如下定义：
定义在样本空间I上，取值于实数域的函数r(w)，称为是样本空间I上的随机变量，并称F(x),(x属于无穷)是随机变量r(w)的概率分布函数。
11、连续型随机变量：[定义]若r(x)是随机变量，F(x)是它的分布函数，如果存在函数p(x)使对任意的x有F(x)=Int p(y)dy(负无穷到x)int表示积分,则称r(x)为连续型随机变量，F(x)为连续型分布函数，p(x)为其概率密度。
12、正太密度：若随机变量服从一个位置参数为u，尺度参数为a的概率分布，且其概率密度函数为：

这个随机变量就称为正太随机变量，其对应的分布称为正太分布。[高斯在研究误差理论时经常用它来刻画误差，经验表明许多实际问题中的变量如测量误差、射击时的偏差、热力学中理想气体的分子速度等都可以认为服从正太分布。进一步证明，一个变量如果受到大量微小的、独立的随机因素的影响
，那么这个变量一般是一个正太变量。]
正太分布转标准正太分布的计算：p118(暂留！)
13、二项分布、伯努利分布、多项分布：进行一个n重伯努利实验，x表示实验成功的次数，概率：
右图二项式系数，其构成的分布为二项分布。而伯努利分布是在n=1时的二项分布特例，所以伯努利分布列中只有0和1两个结果也叫两点分布。多项分布：
多项分布是二项分布的推广，先看一例：城市中按年龄分为n个组，对应的频率为p1,p2,...，现从城市中随机抽取N个人用x1,x2,...xn表示其中各age组所包含的人数，则x1,...xn服从多项分布。X~B(N,P)
定义：如果一个随机向量x=(x1,...xn)满足如下条件：(1)xi>=0(1<=i<=n),且x1+...+xn=N.(2)设m1,...mn
为任意非负整数，且m1+..+mn=N,则事件{x1=m1,x2=m2,...xn=mn}的概率为：
p{x1=m1,...xn=mn}=N!*p1^m1*...*pn^mn  /  m1!...mn!	(p1+p2+...+pn)=1
则称随机向量x=(x1,...xn)服从多项分布，x~PN(N:P1,...Pn)
14、全概率公式：
概率：概率则表示一个样本在总样本中发生的可能性。
### 三、数学分析知识：
...
【定理17.6：若函数f在点p0(x0,y0,z0)可微则f在p0沿任意方向l的方向导数都存在且：fl(p0) = 
fx(p0)cosa+fy(p0)cosb+fz(p0)cosc,cosa,cosb,cosc为方向l的反向余弦。证明可从距离公式开始：
x-x0=d*cosa,y-y0=d*cosb,z-z0=d*cosc】
