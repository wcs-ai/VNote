...
变量与函数的声明并不会被优先提升到其作用域开头，而是按顺序的。
...
**语句作用域**：`if,switch,while,for`语句控制结构内定义变量，定义在控制结构中的变量，这些变量无法在结构外使用。

```c++
if(){}
for(int i=0;i<10;i++){}
while(10<12){}
// switch只能用char类型，不可用string
switch('g'){
    case 'a':
        //...
        break;
...
函数重载：同一作用域里出现多个同名函数，但传参不同，这依然可以使用，称之为函数重载（编译器根据所传参数决定调用哪个执行）

```c++
void hello(int i);
void hello(string j);
void hello(string j,int c);
```

不同作用域中无法重载，函数中声明与外部同名的变量/函数则会屏蔽掉外部的同名变量/函数。
