:::alert-info
想做的项目：一个能与人智能对弈五子棋或象棋的智能程序、机器与机器的博弈、一个集成了图像识别(识别常见事物)，智能聊天，能和人互动的3D美女、撩妹神器、输入联想词、图片文字互转、智能聊天、语音识别、中英互译、根据题目写一篇简短文章。
:::
...
- 长文本-长文本：通过使用主题模型，我们可以得到两个长文本的主题分布，再通过计算两个多项式分布的距离来衡量它们之间的相似度。衡量多项式的距离 可以利用Hellinger Distance

**模式匹配**：又称字符串查找，分为精确匹配和模糊匹配，**精确匹配**常用算法有KMP,BM,BMH等。**模糊匹配**有N-Gram,模糊匹配关键在于衡量两个长的很像的词，这方面有基于编辑距离的概念，人们设计出了 Smith-Waterman 算法和Needleman-Wunsch 算法。模糊匹配还常用于拼写检查。
**N-Gram算法**：先将要计算的两个字符串按长度N(2或3)切分成多个子字符串，按以下公式计算：[学习参考地址](https://www.jianshu.com/p/4452cf120bd7)
`G(s1)+G(s2)-N*|G(s1)nG(s2)|`(两个字符串划分后的子字符串的长度(以N为单位)相加，然后减去N乘以两个字符串交集的长度，如果两个字符串完全一样结果就为0)

**BMH算法**：BMH算法是BM的改进版，属于精确匹配(在匹配串中找到和目标串完全一样的子串)。先从头部开始进行匹配，如果目标串最后一个字符不在当前模板串(匹配串当前使用的子串)内，则直接跳动目标串长度再进行匹配，否则跳动距最后一个字符最近的字符长度再进行匹配。
...
**solr的使用**：solr是开放源代码的，基于Lucene java的搜索服务器。solr依存于Lucene，底层核心技术用Lucene实现，Lucene本质上是个搜索库，不是独立的应用程序，而solr是。
...
<i class="label1">使用</i>语法大致跟mysql一致，不过hive不支持UPDATE，DELET操作。hql限制如下:
