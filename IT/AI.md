...
with open('data/train.txt','w') as data_a,\
    ...
    print(data_a.read())
```
<i class="label2">sys模块</i>：[一个详解教程](https://www.cnblogs.com/fmgao-technology/p/9074282.html)
```
import sys
...
[一个使用依存句法分析的三元信息抽取库](https://github.com/lemonhu/open-entity-relation-extraction)：(其中需要使用pyltp库，直接pip安装即可，[使用到的模型数据文件下载地址](http://ltp.ai/download.html)，下载最左侧的zip包)。[一个基于pytorch的关系抽取模型](https://www.ctolib.com/zjunlp-deepke.html)
