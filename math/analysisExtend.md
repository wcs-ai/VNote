...
注：趋于 ∞ 的可以将其先替换为导数，便于泰勒展开；如：$(n->∞)对(1+1/n)^n,令x=1/n得:e^{ln(1+x)/x}$#再泰勒展开[^TaiLe]
**几个常用泰勒展开如下**：复合泰勒展开，参考[^TLe]
...
**利用单调有界**：求出递推数列的单调性和其上界/下界后，可直接令其极限为 A，然后替换掉式中的 x~n+1~ x~n~为 A，解出 A 即可。
（c）有界的证法：放大/缩小方法、归纳法、不等式。
**其它方法**：进行替换变形、有规律性的，按规律进行求解、画图法、满足 stolz 公式的也可使用。

### j、三角函数趋于无穷的极限
使用三角函数的==周期性来化简，然后等价替换==，求解
- 题1：求$I=\lim_{x→∞}n\tan(\pi\sqrt{n^2+1})=?$
