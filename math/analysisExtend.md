...
2. ==分开积分==：（应首先考虑这一步）被积函数转为和式，然后拆分开积分（一般可以极大的**简化计算**）；
   $$
...
- **轮换对称性**：若积分区域 D 关于 y=x 对称，==被积函数交换积分后值不变==（$\int\int g(x,y)da=\int\int g(y,x)da$）则有如下：
  $$
  ...
  例：\int\int_D x^2-y^2dxdy=\frac{1}{2}\int\int_D x^2-y^2+y^2-x^2 dxdy=0~~,(D关于y=x对称)
  $$

4. **移动区域中心**：积分区域本身对称，但不按 x，y 轴对称，且又难以计算时可以考虑此法（==将积分区域移到圆心==）
   $$
   ...
   $$
   例题思路：（1）极坐标稍作变化，将直角坐标中的1，去掉，让变换后的==区域D圆心在坐标原点==；

   （2）极坐标替换式代入被积函数（3）**r与θ的取值范围**按D圆心变换在坐标原点的情况取；
...
积分中，上下限有无穷（无穷限的反常积分），或被极函数中有使函数发散的点，或分母为0的点（瑕积分）
方法较多，看情况使用，不过==若被积函数发散，则积分就是发散的==
...
- 计算反常积分：（**被积函数简单**的可优先考虑）依然是利用换元法、分部积分法等找到原函数，再代入上下限计算极限。

- **缩放法**：被积函数进行适当的缩小或放大为易知的情况；

- 图像法：被积函数图像容易绘制的情况，可尝试描述其积分区域，直观上就发散的可直接确认；

- 无穷限反常积分中的**比较审敛法**：（找到1个合适的函数与被积函数比较）
  对$\int^{+∞}_af(x)dx$,f(x)在`[a,+∞]`上连续，其非负,若$\lim_{x\rightarrow +∞}\frac{f(x)}{g(x)}=L$则：
  $$
  \begin{cases}L=0时，\int^{+∞}_ag(x)dx收敛，则\int^{+∞}_af(x)dx收敛（大收小必收）\\
  当L=+∞时，\int^{+∞}_ag(x)dx发散，则\int^{+∞}_af(x)dx发散（小发大必发）\\
  0<L<+∞时，它们具有相同敛散性
  \end{cases}
  $$

- **瑕积分中的比较审敛法**：瑕点为c，$\lim_{x\rightarrow c}\frac{f(x)}{g(x)}=L$,判断`方法同上`

- 无穷限反常积分中的**极限审敛法**：
  对$\int^{+∞}_af(x)dx$,f(x)在`[a,+∞]`上连续，其非负，则：
  （1）若存在常数p>1,使得$\lim_{x\rightarrow ∞}x^pf(x)=c<+∞$,则该反常积分收敛；
  （2）如果$\lim_{x\rightarrow ∞}xf(x)=d>0$,则该反常积分发散；
  
- 瑕积分中的==极限审敛法==：
  对$∫^b_af(x)dx$，c为瑕点，,f(x)在`(a,b)`上连续，其非负，则：
  （1）若存在常数0<q<1,使$\lim_{x\rightarrow c}(x-c)^qf(x)$存在，则收敛；
  （2）若$\lim_{x\rightarrow c}(x-c)f(x)=d>0$，则积分发散；

**例**：积分$I=\int^{+∞}_1\frac{dx}{x^p\ln^qx}$(p>0,q>0)收敛，求p值取值范围
解：x=1时分母为0，所以x=1也是其`瑕点`,此处用**比较审敛法**，如下：
$令g(x)=1/(x-1)^q~~,f(x)=1/(x^p\ln^qx)~~,\lim_{x\rightarrow 1}\frac{f(x)}{g(x)}=1$所以f(x)与g(x)有相同的敛散性
则：$\int^{+∞}_1 g(x)dx=\frac{-1}{q+1}\frac{1}{(x-1)^{q-1}}|^{+∞}_1$,`q<1`时收敛
对于∞处，使用**极限审敛法**：$\lim_{x\rightarrow ∞}x^pf(x)=0$,`p>1`时才是收敛
...
3. $C:r=r(\Theta),a\leq\Theta\leq b的弧长s=\int ^b_a\sqrt{r^2+(r')^2}d\Theta$

4. 例：极坐标曲线`θ=(r+1/r)/2`,在对应于`1<=r<=3`之间的弧长？（==注意转为参数方程思路==）
   $$
   两边同时cos/sin化为参数方程\begin{cases}x=rcos[\frac{r+1/r}{2}]\\ y=rsin[\frac{r+1/r}{2}]\end{cases}
   ~~,s=\int^3_1 \sqrt{(x'_r)^2+(y'_r)^2}dr=2+(ln3)/2
   $$
