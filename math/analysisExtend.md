...
- ==泰勒展开求无穷小的阶==：将目标式子展开，最后为==1项因式==(1项可判断展开到几阶) + 高阶无穷小量（此时高阶无穷小的阶数，**即为因式的阶**）
  例：求f(x)=`sinx-x`的阶数；
  解对sinx泰勒展开：$T(sinx)=0+x-x^3/3!+x^5/5!+o(x^5)$,代入f(x)得：$x-x^3/3!+x^5/5!+o(x^5)-x$
  得:$T(f(x))=-x^3/3!+x^5/5!+o(x^5)$（因为==只能有1项因式==，所以只能展开到3阶）
  最后得：$T(f(x))=-x^3/6+o(x^3)$,所以f(x)是**3阶无穷小**
## 1、证明可微可导
...
- **二重积分求导**：==先对外面的一重求导，再对里面的一重积分求导==（注意是否要先交换积分次序再求导）
  **注意**：相关参数要==拿到后面的积分中==，例：$(\int^x_0\int^{u^2}_0f(t,u)dt)'=\int^{x^2}_0f(t,x)dt$
...
- **隐函数求导 1**：<b c=b>由函数 F(x,y)=0 确定的隐函数 y=y(x)</b>
  ...
  **求 y'(a)的值**：将 x=a 代入原方程，得到`y=y(a)`值，再**同时代入**`y'式子`中即可。
  ==这种具象表达式情况求二阶导==：依然用$d^2y/dx^2=\frac{d(dy/dx)}{dy}\frac{dy}{dx}$
...
- **多元隐函数求极值点**：与上类似
  ...
  \end{cases}$$
  由(1),(2)两式，==令==$\frac{\partial z}{\partial y}=0,\frac{\partial z}{\partial x}=0$代入得x,y,z关系式，再代入原方程得其点。
...
例：已知$y_1=xe^x+e^{2x},y_2=xe^x+e^-x,y_3=xe^x+e^2x-e^{-x}$是某二阶线性非齐次微分方程的3个解，求此微分方程。
...
$$(1) \int \frac{1}{cosx}dx=ln|secx+tanx|+c~~(9)\int \frac{1}{sinx}dx=ln|cscx-cotx|+c \\
  ...
  (3)\int cos^2xdx=\frac{x}{2}+\frac{1}{4}sin2x+C$$
...
- **第二换元积分**：直接将自变量替换`(dx->[t=2+4x]->4dt)`，往往替换为熟知的积分公式形式，或便于变形，**是解决多数积分难题的方法**；
  ...
  **cos变sin**：顶积分情况，==cos，sin互换，修改积分上下限==，$\int^{\pi/4}_0ln(cost)dt=\int^{\pi/2}_{\pi/4}ln(sint)dt$
...
5. **用极坐标计算**：先==根据积分上下限==中 x，y 的关系绘制出积分区域。写成二重积分形式，再使用极坐标$T:\begin{cases} x=ρcos\theta \\y=ρsin\theta \end{cases}$
   代入替换，变换形式为：$\int\int ^{}_D f(x,y)dxdy=\int\int ^{}_\Delta ρ*f(ρcos\Theta,ρsin\Theta)dρd\Theta$所以便不是单纯的把微分 dxdy 转为 drdθ
   详情见**数下 p251**。**注意**，θ 范围内若积分区域不是规则的圆（且圆心在原点）==至少有一侧 r 需要用 θ 相关式子表示==
   $$
   ...
   $$
6. 极坐标计算无被积函数情况：$s=\int dθ\int rdr$
## 6、反常积分敛散性判定
...
==一些特殊曲线==：
- 玫瑰线：(3叶玫瑰线$r=asin3θ$,其中**一瓣**的面积：$\frac{\pi}{12}a^2$)
  <img src="./vx_images/meigui.jpeg" style="height:200px;"/>
- 双扭线：极坐标：$r^2=a^2cos(2θ),直角坐标:(x^2+y^2)^2=x^2-y^2,~所围面积:s=2\int^{\pi/4}_0cos2θdθ=a^2$
  <img src="./vx_images/suanniu.jpeg" style="height:200px;"/><img src="./vx_images/xin.webp" style="height:200px;"/><img src="./vx_images/xing.webp" style="height:200px;"/>
- 心形线：$r=a(1+cosθ)$,一周的长为8a
...
- ==绕非x/y轴旋转所得体积==：先求出所围面积S，再用积分思想，$V=\int^{2\pi}_0Sds$
- **极坐标方程旋转体**：给出极坐标方程，求绕极轴旋转的体积
  解：极轴对应到直角坐标系就是==x轴==，将==极坐标方程转为x,y表示==的参数方程，再按公式代入即可
  **例**：心形线$r=4(1+cosθ)与θ=0,θ=\pi/2$所围图形D，求D绕极轴旋转一周所得体积
  解：极坐标方程转为参数方程表示：$\begin{cases}x=4(1+cosθ)cosθ \\ y=4(1+cosθ)sinθ\end{cases},再代入V_x=\pi\int y^2dx$即可
## 3、曲线弧长

1.  $C:y=y(x),a\leq x\leq b的弧长s=\int ^b_a \sqrt{1+(y')^2}dx$
2.  $C:\begin{cases}x=x(t)\\y=y(t)\end{cases},a\leq t\leq b的弧长s=\int ^b_a \sqrt{(x')^2+(y')^2}dt$
3.  $C:p=p(\Theta),a\leq\Theta\leq b的弧长s=\int ^b_a\sqrt{p^2+(p')^2}d\Theta$
...
- **二维图形求质心时**：找出y(x)与x的关系，将公式中的p(x,y)换成y(x)
例1：求$\frac{x^2}{a^2}+\frac{y^2}{b^2}=1~~(0\le x\le a,0\le y\le b)$所成图形的质心。
$$
根据质心公式：x^*=\frac{\int^a_0 xy(x)dx}{\int^a_0 y(x)dx},y^*=\frac{\int^a_0 yy(x)dx}{\int^a_0 y(x)dx}=\frac{\int^a_0 y^2(x)dx}{\int^a_0 y(x)dx} \\
\int^a_0 xy(x)dx=\int^a_0 xb\sqrt{1-x^2/a^2}dx=\frac{a^2b}{3},~~\int^a_0 y^2(x)dx=\int^a_0 b^2(1-x^2/a^2)dx=\frac{2ab^2}{3} \\
分母相当于是图形面积：S=\frac{\pi ab}{4},~~x^*=\frac{\int^a_0 xydx}{S}=\frac{4a}{3\pi}
$$
...
[^TaiLe]: 泰勒展开：有如下两种余项模式：【1】**Peano 余项**(皮亚诺型)：$R_n(x)=o((x-x_0)^n)$，对函数的假设条件较少，只需x0处n阶导即可，不要求n+1阶可导，也==不需在x0领域内存在n阶连续导==当不需要定量讨论余项时，可用皮亚诺余项（如求未定式极限及估计无穷小阶数等问题）【2】**Lagrange 余项**(拉格朗日)：$R_n(x)=\frac{1}{(n+1)!}f^{(n+1)}(ξ)(x-x_0)^{n+1}~,~∃ξ∈(x_0,x)$，注意这`最后一项是ξ`；当需要定量讨论余项时，要用拉格朗日余项（如利用泰勒公式近似计算函数值（==复合函数时：对最外层进行泰勒展开即可，然后x替换为内部的复合函数==）
[^TLe]: 泰勒展开示例：$T[ln(1+x)=x-x^2/2+o(x^2)],复合的T[ln(1+tanx)]=tanx-tan^2x/2+o(x^2)$(替换x即可)