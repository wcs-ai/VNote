...
组合公式：$C^n_m=\frac{A^n_m}{n!}=\frac{m!}{n!(m-n)!},\space\space C^0_n=C^n_n=1$#<b c=r>0 的·阶乘为 1（人为规定）</b>
...
4. **凸函数与凹函数**：国内和国外对两者的定义是相反的。
   ...
   **几何意义**：凸函数上两点 x1<x2，(x1,x2)这段**曲线上的点**总是**在直线段 x1-x2 下方**（图像是向下凹的）；
   定理 1：定义 2 与定义 3 等价；
...
5. **黎曼函数**：即约分数即为最简分数。在很多情况下可以**作为反例来验证某些函数**方面的待证命题。
   ...
   特征：黎曼函数是区间上的有界函数，其上确界是 1/2，下界是 0、黎曼函数在有理点的图象关于直线 x=1/2 对称。
...
- **渐进线**：当曲线上一点 M 沿曲线无限远离原点或无限接近间断点时，如果 M 到一条直线的距离无限趋近于零，那么这条直线称为这条曲线的渐近线。**并不是所有曲线都有渐近线**，渐近线反映了某些曲线在**无限延伸时的变化情况**。按方向渐近线分为水平渐进线、垂直渐近线、斜渐进线。

  1. 水平：如果当`x→∞时,f(x)→c`,则曲线 y=f(x)有一**水平渐近线**y=c。
